...
d3.csv("data/crash_data_n.csv", function(data) {
...
var bubble_chart = (function(d3, DisplayTooltip) {
 ...
 var no_fatalities = {
  ...
  var fill_color = d3.scale.ordinal()
      ...
      .range(["#f2f2f2", "#ccc", "#ccc", "#333"])


  function custom_chart(data) {
    var max_amount = d3.max(data, function(d) { return parseInt(d.vehicles_involved, 36); } );
    radius_scale = d3.scale.pow().exponent(0.6).domain([0, max_amount]).range([1, 9]);

    data.forEach(function(d){
      var node = {
        id: d.id,
        radius: radius_scale(parseInt(d.vehicles_involved, 35)),
  ...
  function charge(d) {
    return -Math.pow(d.radius, 2.6) / 1;
  }
...
/**** *******/
  ...
  function show_details(data, i, element) {
    ...
    .attr("stroke", "black")
    .attr("fill", "black");
    var content = "<h3>" + data.location + "</h3>";
    content +="<span>" + data.weather_cond + "<span/><br>";
    content +="<span>" + data.road + "<span/><br>";
    content +="<span>" + data.name_asset + "<span/>";
